...
class ir_model(osv.osv):
    ...
    def init(self, cr):
        ...
        cash_groups = [  # Group that won't be readonly
            'Accountant',
            'Financial Manager',
        ...
        am_model = ['account.move','account.move.line']
        
        query1 = """
            update ir_model_access
            set  perm_read = True, perm_write = True, perm_unlink = False, perm_create = True
            where
            group_id  in (select id from res_groups where name in
            %s)
            and model_id in (select id from ir_model where model in %s)
        """
        cr.execute(query1, (tuple(inc_groups), tuple(am_model)))
        
        cash_model = ['account.bank.statement']
        ...
        query3 = """
            ...
            and model_id in (select id from ir_model where model in %s)
        """
        cr.execute(query3, (tuple(cash_groups), tuple(cash_model)))
        
        
        petty_model = ['petty.cash']
        petty_grp = ['Accountant']
        
        query4 = """
            update ir_model_access
            set  perm_read = False, perm_write = False, perm_unlink = True, perm_create = True
            where
            group_id  in (select id from res_groups where name in
            %s)
            and model_id in (select id from ir_model where model in %s)
        """
        cr.execute(query4, (tuple(petty_grp), tuple(petty_model)))
